...
pub const Dir = struct {
    ...
    pub const Walker = struct {
        ...
        pub const WalkEntry = std.Io.Dir.Walker.Entry;

        pub fn next(self: *Walker) !?WalkEntry {
            return try self.inner.next(shared.io());
        }
    ...
    pub fn walk(self: Dir, allocator: Allocator) Allocator.Error!Walker {
        return .{ .inner = try self.toInner().walk(allocator) };
    }
...
pub fn copyFileAbsolute(source_path: []const u8, dest_path: []const u8, options: Dir.CopyFileOptions) std.Io.Dir.CopyFileError!void {
    const source_dir_path = std.fs.path.dirname(source_path) orelse unreachable;
    const source_basename = std.fs.path.basename(source_path);
    const dest_dir_path = std.fs.path.dirname(dest_path) orelse unreachable;
    const dest_basename = std.fs.path.basename(dest_path);

    var source_dir = try openDirAbsolute(source_dir_path, .{});
    defer source_dir.close();
    var dest_dir = try openDirAbsolute(dest_dir_path, .{});
    defer dest_dir.close();
    try source_dir.copyFile(source_basename, dest_dir, dest_basename, options);
}
...
pub fn symLinkAbsolute(target_path: []const u8, sym_link_path: []const u8, flags: Dir.SymLinkFlags) !void {
    const dir_path = std.fs.path.dirname(sym_link_path) orelse unreachable;
    const basename = std.fs.path.basename(sym_link_path);
    var dir = try openDirAbsolute(dir_path, .{});
    defer dir.close();
    try dir.symLink(target_path, basename, flags);
}
