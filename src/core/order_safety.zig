...
pub fn evaluate(order: orders_mod.Order, events: []const state_mod.Event) Summary {
    ...
    for (events) |event| {
        ...
        if (isSuccessEvent(order, event)) {
            ...
            if (safe_executions >= required_safe_executions) {
    ...
    const probation = active and !circuit_open and cleared_event_id == 0;
...
pub fn isGuardedOrder(order: orders_mod.Order) bool {
    return std.ascii.eqlIgnoreCase(order.kind, "trigger") or
        std.ascii.eqlIgnoreCase(order.kind, "mandate");
}
...
test "order safety requires consecutive safe executions to clear probation" {
    var events = [_]state_mod.Event{
        .{
            .id = 1,
            .space_id = "ops",
    ...
    const interrupted = evaluate(order, events[0..4]);
    try std.testing.expectEqualStrings("probation", interrupted.status);
    try std.testing.expect(interrupted.probation);
    try std.testing.expectEqual(@as(usize, 1), interrupted.safe_executions);
    try std.testing.expectEqual(@as(usize, 0), interrupted.consecutive_failures);
    try std.testing.expectEqual(@as(u64, 3), interrupted.last_failure_event_id);

    const clear = evaluate(order, events[0..]);
    try std.testing.expectEqualStrings("clear", clear.status);
    try std.testing.expect(!clear.probation);
    try std.testing.expectEqual(@as(usize, 2), clear.safe_executions);
}

test "schedule orders report safety as not applicable until breaker enforcement exists" {
    var events = [_]state_mod.Event{
        .{
            .id = 1,
            .space_id = "ops",
    ...
    const summary = evaluate(order, events[0..]);
    try std.testing.expectEqualStrings("not_applicable", summary.status);
    try std.testing.expect(!summary.guarded);
    try std.testing.expect(!summary.probation);
    try std.testing.expect(!summary.circuit_open);
}
