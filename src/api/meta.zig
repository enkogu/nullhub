...
const routes = [_]RouteSpec{
    ...
    .{
        ...
        .category = "orders",
        .summary = "Transition Order status with product verbs; activate emits order.activated and pause emits order.paused. Active mandate Orders require a goal. Legacy enact/suspend aliases are accepted.",
        .auth_mode = "optional_bearer",
        .path_params = order_id_params[0..],
        .query_params = order_query_params[0..],
        .response = "Updated Order record with derived safety state when event evidence is available.",
